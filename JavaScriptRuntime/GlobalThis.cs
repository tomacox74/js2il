...
namespace JavaScriptRuntime
{
    ...
    public static class GlobalThis
    {
        ...
        // Host-provided timer function values.
        // These are exposed via GetFunctionValue(name) so libraries can treat timers as first-class
        // globals (e.g., window.setTimeout = setTimeout).
        private static readonly JsFunc2 _setTimeoutFunctionValue = static (_, callback, delay) =>
        {
            var args = RuntimeServices.GetCurrentArguments() ?? System.Array.Empty<object?>();
            var restCount = System.Math.Max(0, args.Length - 2);
            if (restCount == 0)
            {
                return setTimeout(callback!, delay!, System.Array.Empty<object>());
            }

            var rest = new object[restCount];
        ...
        private static readonly JsFunc1 _clearTimeoutFunctionValue = static (_, handle) => clearTimeout(handle!);

        private static readonly JsFunc2 _setIntervalFunctionValue = static (_, callback, delay) =>
        {
            var args = RuntimeServices.GetCurrentArguments() ?? System.Array.Empty<object?>();
            var restCount = System.Math.Max(0, args.Length - 2);
            if (restCount == 0)
            {
                return setInterval(callback!, delay!, System.Array.Empty<object>());
            }
        ...
        private static readonly JsFunc1 _clearIntervalFunctionValue = static (_, handle) => clearInterval(handle!);

        private static readonly JsFunc1 _setImmediateFunctionValue = static (_, callback) =>
        {
            var args = RuntimeServices.GetCurrentArguments() ?? System.Array.Empty<object?>();
            var restCount = System.Math.Max(0, args.Length - 1);
            if (restCount == 0)
            {
                return setImmediate(callback!, System.Array.Empty<object>());
            }
        ...
        private static readonly JsFunc1 _clearImmediateFunctionValue = static (_, handle) => clearImmediate(handle!);

        public static object GetFunctionValue(string name)
        {
            if (string.IsNullOrWhiteSpace(name))
            {
                throw new ArgumentException("Expected a global function name.", nameof(name));
            }

            switch (name.Trim().ToLowerInvariant())
            {
                case "settimeout":
                    return _setTimeoutFunctionValue;

                case "cleartimeout":
                    return _clearTimeoutFunctionValue;

                case "setinterval":
                    return _setIntervalFunctionValue;

                case "clearinterval":
                    return _clearIntervalFunctionValue;

                case "setimmediate":
                    return _setImmediateFunctionValue;

                case "clearimmediate":
                    return _clearImmediateFunctionValue;

                default:
                    throw new NotSupportedException($"Global function value '{name}' is not supported.");
            }
        }
