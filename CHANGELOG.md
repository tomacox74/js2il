...
- fix(runtime): pack JavaScript arguments into CLR `params` arrays during
  ...
  every runtime uses the common upstream-derived scenario source.
...
- test262/docs: verify 500 additional class-declaration parse-negative and early-error
  ...
  metadata-selected variants and all 500 native tests pass without compiler, runtime,
  or harness changes.
...
- test262/docs: port 183 additional `Array.prototype.some` cases covering
  ...
  mutation during iteration, metadata, and abrupt completion. Twenty-five
  exotic property/prototype, resizable-buffer, and direct-`eval` cases remain
  deferred for focused follow-up work.
...
- runtime/test262/docs: port 125 previously failing Array conformance tests
  ...
  `toString`; preserve fast paths for true arrays while adding specification
...
- runtime/node: add callable `assert` and `node:assert` support with core scalar,
  ...
  module contract. Use it for test262 assertions through a compatibility
  adapter, validate expected exception constructors, and replace assertion
  output snapshots with thrown test failures.
...
- sdk/runtime: add shared generated nested Node contract support with stable
  ...
  options/FileHandle flows now use the generated contracts and direct adapters.
...
- sdk/runtime: add a generated `node:process` host contract from the pinned
  ...
  APIs.
- sdk/runtime: add generated `node:buffer`, `node:events`, and `node:os` host
  contracts from pinned official Node.js 24.18.1 documentation. Bind existing
...
- sdk/runtime: add generated `node:zlib`, `node:string_decoder`, `node:timers`,
  and `node:timers/promises` host contracts from pinned official Node.js
...
- compiler/runtime: strongly type literal CommonJS requires for generated Node
  ...
  their nested contract types through captured scope fields and direct member
  calls, while preserving dynamic fallback after aliasing, mutation, deletion,
  reflection, or escape; dynamic and non-contract requires retain the existing
  dispatcher.
...
- compiler: retire the legacy Stackify scheduling/adjacency pass. The validated
  ...
  `TempLocalAllocator` retains ordinary spills and compatible-slot reuse.
  Removes the duplicate stack-effect inventory, generic stack simulation,
  typed-numeric adjacency guard, and Stackify-specific call deferral; updates
  ownership tests and contributor guidance for default-deny effects, canonical
  metadata, residency, validation, and symbol preservation. Scope and getter
  reads now remain materialized unless the scheduler proves residency, fixing
  legacy getter/setter evaluation order. Single-use scope reads can now remain
  stack-resident when they are the leading operand of a validated receiver
  operation; adjacent call argument/scope arrays use scheduler-inline
  construction, and normalization removes unused compiler-internal scope-array
  producers. Immutable local binary operands no longer receive redundant
  pre-RHS snapshot copies.
- runtime: make `ArgumentsObject` inherit `Object.prototype`, restoring ordinary inherited-property behavior and correct prototype identity. Ports ten upstream test262 arguments-object cases covering mapped and unmapped parameter aliasing, indexed property mutation/deletion and descriptors, `length`, and non-strict `callee` descriptors.
- compiler: generalize LIR scheduling into a deterministic consumer-rooted topological planner within validated straight-line/source regions. Complete contiguous single-use DAGs built from previously proven arithmetic, comparison, conversion, stable-load, literal, and call producers can now execute inside nested construction/argument roots, eliminating residual locals in mixed Math trees and nested array/object/call values. Explicit data, effect/may-throw, region, variable-slot, operand-order, and supported-family checks preserve JavaScript evaluation and allocation order; alias-sensitive getters/stores, fallback/dynamic calls, spread/computed forms, multi-use/non-contiguous trees, and every control/sequence/EH/suspend/scope/unknown boundary fail closed. Adds deterministic plan, acceptance/rejection/residual metrics, hard-boundary, exact IL, effect/alias order, PDB, stack-trace, and IL-verification coverage.
- compiler: keep supported single-use call results on the CLR evaluation stack. Normalized intrinsic-static, immediate typed-member, and direct user-class calls now execute exactly once at their source operation and can feed arithmetic, comparisons, conversions, nested direct calls, or synchronous returns without `stloc`/`ldloc`; ordered non-spread `Math.max`/`min`/`hypot` argument calls use scheduler-inline params-array construction to avoid invalid values below `newarr`. Multi-use, fallback, spread, non-contiguous, variable-backed, sequence-point/control/EH, and async/generator shapes fail closed. Retires Stackify's typed-call deferral and adds exact IL, effect order, exception, maxstack, PDB/stack-trace, fluent typed-call, and materialization-negative coverage.
- compiler: add the first non-identity LIR schedule for literal and intrinsic argument construction. Supported single-use producer trees are emitted exactly once inside ordered `object[]`, `Array`, and object-literal construction units, eliminating position-dependent spills in calls such as `Math.max(a * 2, b + 3)` and in array/object values while preserving left-to-right evaluation, allocation identity, exception order, source intervals, and final empty-stack validation. Adds explicit scheduler-inline residency, schedule-effective effect ordering/liveness, linear operation reconstruction, exact IL and nested-construction coverage, and fail-closed handling for calls/getters, computed/effectful properties, spread/iterator lowering, TDZ/mutable loads, and non-contiguous trees.
- compiler: extend cumulative LIR stack scheduling to numeric/object conversions, string concatenation, and statically proven string, `Array`, and `Int32Array` length/element loads. Covered single-use definitions now execute once at their source position and feed supported operations or synchronous returns without intermediate locals, while generic getters, mutable variable slots, TDZ-checked loads, calls, allocations, and async/generator return shapes remain legacy-owned. Extracts allocator and Stackify reproduction decisions behind `LIRRematerializationPolicy`, making stack residency, rematerialization, and local materialization explicit and mutually exclusive. Adds exact IL and execution coverage for conversions, concat, typed loads, repeated constants, postfix snapshots, getter counts, and TDZ behavior.
- compiler: extend cumulative LIR stack scheduling to typed numeric unary operations and number/Boolean comparisons. Covered negation, bitwise-not, `<`, `>`, `<=`, `>=`, `==`, and `!=` results execute once at their source position and can feed numeric/comparison trees, synchronous returns, or immediate branches without intermediate locals. Scheduler-owned branch comparisons bypass legacy inline branch fusion, while unsupported shapes remain legacy-owned. Adds exact no-local IL, branch-stack, scheduler-ownership, NaN/infinity/equality, signed-zero negation, call-order, TDZ, PDB, and IL verification coverage.
- compiler: enable the first optimizing LIR stack-scheduler mode for single-use typed numeric `+`, `-`, `*`, `/`, `%`, and `**` expression trees. Numeric producers execute once at their original LIR position, remain on the CLR evaluation stack, and feed nested numeric operations or safe terminal consumers without intermediate `stloc`/`ldloc` or rematerialization. Candidate selection preserves source order, expression grouping, operand position, IEEE-754 behavior, sequence-point/control boundaries, call order, postfix snapshots, and legacy fallback for unsupported shapes. Adds exact IL, scheduler-metric, NaN, signed-zero, non-commutative, call-order, and snapshot coverage; promotes the intentional IL improvements across existing generator snapshots. Temp-local release is now idempotent per temp/instruction, preventing duplicate operands such as `x + x` from freeing one slot twice and aliasing later overlapping live ranges.
- compiler: complete the scheduler Stage 0 Portable PDB gate. Each scheduling region now records its active sequence-point ordinal and source span, and validation rejects instructions moved outside that source interval. Adds disabled-versus-identity decoded PDB equivalence for documents, ordered spans, source locals, and final `LocalScope` lengths; verifies async/generator statement lines, source-mapped arithmetic exceptions, and compilation with or without PDBs. Scheduler decisions remain independent of `EmitPdb`, while final symbol registration continues to use actual emitted IL offsets, local signature, method length, and stable source-local indexes.
- compiler: make temp residency ownership and local allocation schedule-aware. A new `TempMaterializationPlan` gives each temp exactly one owner across the scheduler, branch-condition fusion, legacy Stackify, rematerialization, variable slots, snapshot barriers, await/yield resume results, catch results, and constructor-return overrides. Branch fusion and Stackify now exclude already-owned scheduler temps; `TempLocalAllocator` walks validated schedule order, uses canonical effective last uses for optimized modes, and preserves legacy identity allocation byte-for-byte. Compatible slots are reused only for non-overlapping scheduled ranges, stack-resident temps receive no local, and async/generator spill/restore continues to use the final slot mapping. Adds local-persistence regressions across `await` and `yield`.
- compiler: add independent LIR stack-schedule validation and schedule-aware maxstack accounting. Plans now prove exact LIR ownership, atomic fusion ownership, effect order, conservative region membership, scheduler/temp ownership, single-def/single-use stack residency, LIFO operand order, boundary stack emptiness, catch-handler exception input, and materialized await/yield resume results before IL emission. Validated carried depth is added to the existing instruction-internal peak estimator; Debug/test mode fails strictly, while Release can record a rejection and fall back to a freshly validated identity schedule. Restores arbitrary-value try/catch/finally to the IL verification corpus and adds a deep arithmetic verifier fixture.
...
- perf/compiler: coalesce redundant LIR temp-to-variable-slot copies. The HIR-to-LIR lowerer materializes every variable assignment as a producer temp followed by a `LIRCopyTemp` into a slot-pinned temp, which forced the IL emitter to allocate a throwaway local and emit a `stloc`/`ldloc`/`stloc` shuffle for each one. A new `LIRVariableSlotCoalescing` pass pins the producer's result directly to the destination variable slot when the copy is adjacent to its single definition, both temps are single-use/single-def, and their storage representations match exactly, so the degenerate copy disappears. Callable creation, `let`/`const`/`var` initializers, and assignment expressions all shed a local and two IL instructions apiece.
...
- runtime: add `%TypedArray%.prototype.with` for supported non-BigInt TypedArray families, with immutable same-type copying, index and replacement coercion ordering, negative indexing, standard metadata, concrete prototype wiring, and covered default-species observation. Ports twenty-five upstream test262 cases.
