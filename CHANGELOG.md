...
- compiler/metadata: add object literal type-generation groundwork (issue #1430, parent #1428). Eligible `ObjectLiteralShapeInfo` instances now get deterministic generated CLR helper TypeDefs under the `ObjectLiterals` namespace, with public fields registered in `VariableRegistry` for later construction/direct-access phases; `JsObject` is unsealed so generated literal types can derive from it while preserving dictionary, descriptor, enumeration, and JSON behavior through base-storage mirroring.
