...
- Equality comparisons: fixed object-to-object equality comparisons by adding type coercion when comparing two non-literal object values (e.g., object property access vs method return value). When both operands are objects loaded from variables or expressions (not literals), the IL generator now converts both to numbers using `TypeUtilities.ToNumber()` before comparison, ensuring value equality instead of reference equality. Added test case `BinaryOperator_EqualObjectPropertyVsMethodReturn`. Note: Some edge cases with dynamic property lookups may still require reference equality and are handled separately.
