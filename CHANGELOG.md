...
- Node/path tests: re-enable previously skipped generator snapshots for `path.parse`/`path.format`, `path.normalize`/`path.sep`, and `path.toNamespacedPath` with cross-platform snapshot scrubbing.
