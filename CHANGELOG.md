...
- runtime: migrate short-lived built-in result/iterator objects from `ExpandoObject` to real ordinary `JsObject` instances (issue #1451, parent #1443). Script-visible iterator result objects (`{value, done}`) returned from `Array`/`Map`/`Set`/`TypedArray`/`String` iterators, generator `.next()`/`.throw()`/`.return()`, and the shared `Iterator.prototype`/`Iterator` helper dispatch (`Iterator.from`, `.map`, `.filter`, `.take`, `.drop`, etc.) now go through a new `IteratorResult.ToOrdinaryObject(...)` helper that builds the result via `Object.CreateOrdinaryObject()`, correcting previously-broken behavior: `Object.getPrototypeOf(result) === Object.prototype` (was `null`/incorrect), `Object.keys(result)` now returns `["value","done"]` in spec order (was reversed), and `Object.getOwnPropertyDescriptor(result, "done")` now returns a real descriptor (was `undefined`). `JSON.parse` object results are now built the same way instead of via `ExpandoObject`/raw dictionary manipulation, fixing the same prototype/descriptor gaps for parsed objects.
  - Internal/hot-path iterator consumption (`for-of`, spread, destructuring, `yield*`, `IteratorClose`) continues to use the typed `IJavaScriptIterator.Next()`/`IIteratorResult` interface directly and is unaffected: `GeneratorObject`'s `next()`/`@throw()`/`@return()` were restructured into private `*Core` methods so the internal interface implementations call the unconverted core logic while only the JS-visible wrapper methods pay the ordinary-object conversion cost.
  - Explicitly out of scope (pre-existing gaps, not touched by this migration): named RegExp capture groups (`match.groups`, always `undefined` today), `JSON.parse` reviver parameter (unsupported), async generators/async iterators (`AsyncGeneratorObject`/`IJavaScriptAsyncIterator`, deferred to a future issue since their `next()` resolves a `Promise` deep inside compiler-emitted async state-machine code), global prototypes (`Map.prototype`, `Set.prototype`, `String.prototype`, `RegExp.prototype`, `StringIteratorPrototype`), `import.meta` (ESM), Node APIs (`Node/Url.cs` `SearchParamsIterator`), and the test262 host harness (`Test262HostRuntimeIntrinsics`).
  - Measured locally (Release, precompiled): `for-of` iteration over generators and `Map` (the dominant real-world usage pattern, using the unconverted internal fast path) shows no measurable regression (~925 ms / ~4.3 s for 500k/100k-entry loops, before and after). Explicit script-level `generator.next()` calls are ~2x slower (5.1 s → 10.7 s for 500k calls) and `JSON.parse` on a small nested object is ~2.9x slower (2.8 s → 8.1 s for 200k parses), both due to the cost of allocating a real `JsObject` with registered property descriptors instead of a reflection-accessed POCO/raw dictionary — an accepted correctness-for-allocation trade-off per this project's priorities (spec/Node compatibility over raw performance).
