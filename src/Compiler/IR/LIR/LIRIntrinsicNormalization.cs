...
internal static class LIRIntrinsicNormalization
{
    public static void Normalize(MethodBodyIR methodBody, ClassRegistry? classRegistry, ICallableDeclarationReader? callableReader = null)
    {
        ...
        FuseNumericCandidateConversionsIntoConsumers(methodBody);
    }
    ...
    private static void FuseNumericCandidateConversionsIntoConsumers(MethodBodyIR methodBody)
    {
        ...
        for (var i = 0; i < methodBody.Instructions.Count; i++)
        {
            if (TempLocalAllocator.TryGetDefinedTemp(methodBody.Instructions[i], out var defined)
                && defined.Index >= 0)
            {
                definitions[defined.Index] = i;
            }
        }

        var indicesToRemove = new HashSet<int>();

        for (var consumerIndex = 0; consumerIndex < methodBody.Instructions.Count; consumerIndex++)
        {
            if (!TryGetNumericBinaryOperands(
                methodBody.Instructions[consumerIndex],
                out var left,
                out var right,
                out var replaceOperands))
            {
                continue;
            }

            var leftCandidate = TryGetCandidateConversion(left, out var leftSource, out var leftConversionIndex);
            var rightCandidate = TryGetCandidateConversion(right, out var rightSource, out var rightConversionIndex);
            if (!leftCandidate && !rightCandidate)
            {
                continue;
            }

            var leftDirectNumber = IsUnboxedDouble(left);
            var rightDirectNumber = IsUnboxedDouble(right);
            if (!(leftCandidate && (rightCandidate || rightDirectNumber))
                && !(rightCandidate && (leftCandidate || leftDirectNumber)))
            {
                continue;
            }

            var conversionIndices = new HashSet<int>();
            if (leftCandidate)
            {
                conversionIndices.Add(leftConversionIndex);
            }
            if (rightCandidate)
            {
                conversionIndices.Add(rightConversionIndex);
            }

            var firstConversionIndex = conversionIndices.Min();
            if (consumerIndex <= firstConversionIndex
                || methodBody.Instructions
                    .Skip(firstConversionIndex)
                    .Take(consumerIndex - firstConversionIndex)
                    .Select((instruction, offset) => (instruction, index: firstConversionIndex + offset))
                    .Any(entry => !conversionIndices.Contains(entry.index)))
            {
                continue;
            }

            if ((leftCandidate && IsTempUsedOutside(
                    methodBody,
                    left,
                    new HashSet<int> { leftConversionIndex, consumerIndex }))
                || (rightCandidate && IsTempUsedOutside(
                    methodBody,
                    right,
                    new HashSet<int> { rightConversionIndex, consumerIndex })))
            {
                continue;
            }

            methodBody.Instructions[consumerIndex] = replaceOperands(
                leftCandidate ? leftSource : left,
                rightCandidate ? rightSource : right);

            indicesToRemove.UnionWith(conversionIndices);
        }
        ...
        var compacted = new List<LIRInstruction>(methodBody.Instructions.Count - indicesToRemove.Count);
        for (var i = 0; i < methodBody.Instructions.Count; i++)
        {
            if (!indicesToRemove.Contains(i))
            {
                compacted.Add(methodBody.Instructions[i]);
            }
        }

        methodBody.Instructions.Clear();
        methodBody.Instructions.AddRange(compacted);

        bool TryGetCandidateConversion(
            TempVariable operand,
            out TempVariable source,
            out int conversionIndex)
        {
            source = default;
            conversionIndex = -1;
            if (!definitions.TryGetValue(operand.Index, out conversionIndex)
                || methodBody.Instructions[conversionIndex] is not LIRConvertToNumber conversion
                || conversion.Source.Index < 0
                || conversion.Source.Index >= methodBody.TempStorages.Count)
            {
                return false;
            }

            var sourceStorage = methodBody.TempStorages[conversion.Source.Index];
            if (sourceStorage.Kind != ValueStorageKind.UnboxedValue
                || sourceStorage.ClrType != typeof(JavaScriptRuntime.NumericIndexedValue))
            {
                return false;
            }
        ...
        bool IsUnboxedDouble(TempVariable operand)
            => operand.Index >= 0
                && operand.Index < methodBody.TempStorages.Count
                && methodBody.TempStorages[operand.Index] is
                {
    ...
    private static bool TryGetNumericBinaryOperands(
        ...
        out TempVariable left,
        out TempVariable right,
        out Func<TempVariable, TempVariable, LIRInstruction> replaceOperands)
    {
        switch (instruction)
        {
            case LIRAddNumber value:
                left = value.Left;
                right = value.Right;
                replaceOperands = (newLeft, newRight) => value with { Left = newLeft, Right = newRight };
                return true;
