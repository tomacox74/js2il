...
public sealed partial class HIRToLIRLowerer
{
            ...
            case HIRVariableExpression varExpr:
                ...
                ValueStorage GetPreferredBindingReadStorage(BindingInfo b)
                {
                    ...
                    if (b.IsStableType)
                    {
                        ...
                        if (b.ClrType != null && !b.ClrType.IsValueType)
                        {
                            return new ValueStorage(ValueStorageKind.Reference, b.ClrType);
                        }
                    }
    ...
    private bool TryCreateCallableIdForStableFunctionBinding(
        Symbol symbol,
        ...
        out Scope bodyScope)
    {
        ...
        if (_scope == null
            ...
            || sourceCall == null
            || !IsStableBindingDefinitelyInitializedAtCall(symbol.BindingInfo, initializer, sourceCall))
        {
            return false;
        }
        ...
        var callableScope = FindScopeByDeclarationNode(initializer, root);
        if (callableScope == null
            || callableScope.Parent == null
        ...
        bodyScope = callableScope;
        callableId = initializer switch
        {
            ArrowFunctionExpression arrow => new TwoPhase.CallableId
            {
                Kind = TwoPhase.CallableKind.Arrow,
                DeclaringScopeName = declaringScopeName,
                Name = null,
                Location = TwoPhase.SourceLocation.FromNode(arrow),
                JsParamCount = CountNonRestParameters(arrow.Params),
                NeedsArgumentsObject = false,
                HasRestParameters = callableScope.HasRestParameters,
            ...
            FunctionExpression function => CreateStableFunctionExpressionCallableId(
                function,
                callableScope,
                declaringScopeName),
            _ => throw new InvalidOperationException($"Unexpected stable callable initializer {initializer.Type}.")
        };
        return true;
    }

    private static TwoPhase.CallableId CreateStableFunctionExpressionCallableId(
        FunctionExpression function,
        Scope functionScope,
        string declaringScopeName)
    {
        var isStrict = Jroc.Utilities.ArgumentsObjectSemantics.IsStrictScope(functionScope);
        return new TwoPhase.CallableId
        {
            Kind = TwoPhase.CallableKind.FunctionExpression,
            DeclaringScopeName = declaringScopeName,
            Name = (function.Id as Identifier)?.Name,
            Location = TwoPhase.SourceLocation.FromNode(function),
            JsParamCount = CountNonRestParameters(function.Params),
            NeedsArgumentsObject = functionScope.NeedsArgumentsObject,
            HasRestParameters = functionScope.HasRestParameters,
            UsesMappedArgumentsObject = Jroc.Utilities.ArgumentsObjectSemantics.UsesMappedArgumentsObject(functionScope),
            ArgumentsParameterNames = Jroc.Utilities.ArgumentsObjectSemantics.GetMappedParameterNames(functionScope),
            IncludeCalleeInArgumentsObject = functionScope.NeedsArgumentsObject && !isStrict,
            HasRestrictedFunctionProperties = isStrict,
    ...
    private static bool StableCallableRequiresFunctionObjectInvocation(Node initializer, Scope callableScope)
    {
        ...
        {
            ArrowFunctionExpression arrow => UsesLexicalInvocationContext(arrow),
            FunctionExpression function =>
                Jroc.Utilities.ArgumentsObjectSemantics.IsStrictScope(callableScope)
        ...
        bool Visit(Node node, Node? parent, bool isRoot, int nestedArrowDepth)
        {
            ...
            if (node is Identifier identifier
                ...
                && (string.Equals(identifier.Name, "arguments", StringComparison.Ordinal)
                    || namedFunctionExpressionName != null
                        && string.Equals(identifier.Name, namedFunctionExpressionName, StringComparison.Ordinal)))
            {
                return true;
            }
    ...
    private static bool IsIdentifierValueReference(Identifier identifier, Node? parent)
    {
        return parent switch
        {
            ...
            FunctionExpression function when ReferenceEquals(function.Id, identifier)
                || function.Params.Any(parameter => ReferenceEquals(parameter, identifier)) => false,
            ArrowFunctionExpression arrow when arrow.Params.Any(parameter => ReferenceEquals(parameter, identifier)) => false,
            ...
            MethodDefinition method when !method.Computed && ReferenceEquals(method.Key, identifier) => false,
            PropertyDefinition property when !property.Computed && ReferenceEquals(property.Key, identifier) => false,
            _ => true
        };
    }
