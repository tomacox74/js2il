    ...
    private void EmitLoadTemp(TempVariable temp, InstructionEncoder ilEncoder, TempLocalAllocation allocation, MethodDescriptor methodDescriptor)
    {
        ...
        switch (def)
        {
            ...
            case LIRGetItem getItem:
                {
                    ...
                    {
                        // Emit inline: call JavaScriptRuntime.ObjectRuntime.GetItem(object, object)
                        EmitLoadTempAsObject(getItem.Object, ilEncoder, allocation, methodDescriptor);
            ...
            case LIRGetItemAsNumber getItemAsNumber:
                {
                    ...
                    if (indexStorage.Kind == ValueStorageKind.UnboxedValue && indexStorage.ClrType == typeof(double))
                    {
                        // Emit inline: call float64 JavaScriptRuntime.ObjectRuntime.GetItemAsNumber(object, double)
                    ...
                    {
                        // Emit inline: call float64 JavaScriptRuntime.ObjectRuntime.GetItemAsNumber(object, object)
                        EmitLoadTempAsObject(getItemAsNumber.Object, ilEncoder, allocation, methodDescriptor);
            ...
            case LIRGetItemAsNumberString getItemAsNumberString:
                {
                    // Emit inline: call float64 JavaScriptRuntime.ObjectRuntime.GetItemAsNumber(object, string)
            ...
            case LIRCallFunction callFunc:
                {
                        ...
                        || callableId.HasRestParameters)
                    {
                        ...
                        ilEncoder.Token(_bclReferences.ObjectType);
                        for (var index = 0;
                             index < callFunc.Arguments.Count;
                             index++)
                        {
                            ilEncoder.OpCode(ILOpCode.Dup);
                            ilEncoder.LoadConstantI4(index);
                            EmitLoadTempAsObject(
                                callFunc.Arguments[index],
                                ilEncoder,
                                allocation,
                                methodDescriptor);
                            ilEncoder.OpCode(ILOpCode.Stelem_ref);
                    ...
                    {
                        var parameterClrType =
                            signature?.ParameterClrTypes != null
                            && i < signature.ParameterClrTypes.Count
                                ? signature.ParameterClrTypes[i]
                                : null;
                        EmitLoadTempAsParameterType(
            ...
            case LIRSetItem setItem:
                {
                        ...
                        && indexStorage.ClrType == typeof(double))
                    {
                        ...
                        var setItemMethod = _memberRefRegistry.GetOrAddMethod(
                            typeof(JavaScriptRuntime.ObjectRuntime),
                            nameof(JavaScriptRuntime.ObjectRuntime.SetItem),
                            parameterTypes: new[] { typeof(object), typeof(double), typeof(object) });
                        ilEncoder.OpCode(ILOpCode.Call);
                        ilEncoder.Token(setItemMethod);
                    }
                    else if (indexStorage.Kind == ValueStorageKind.Reference && indexStorage.ClrType == typeof(string))
            ...
            case LIRSetJsArrayLength setArrayLength:
                {
                    // Inline the assignment-expression result of setting Array.length.
                    var receiverStorage = GetTempStorage(setArrayLength.Receiver);
                    if (receiverStorage.Kind == ValueStorageKind.Reference && receiverStorage.ClrType == typeof(JavaScriptRuntime.Array))
                    {
                        EmitLoadTemp(setArrayLength.Receiver, ilEncoder, allocation, methodDescriptor);
                    }
                    else
                    {
                        EmitLoadTempAsObject(setArrayLength.Receiver, ilEncoder, allocation, methodDescriptor);
                        ilEncoder.OpCode(ILOpCode.Castclass);
                        ilEncoder.Token(_typeReferenceRegistry.GetOrAdd(typeof(JavaScriptRuntime.Array)));
                    }
        ...
        MethodDescriptor methodDescriptor)
    {
        switch (instruction)
        {
            ...
            case LIRExpNumber expNumber:
                ...
                EmitNormalizeExponentiationBase(ilEncoder);
                EmitLoadTemp(expNumber.Right, ilEncoder, allocation, methodDescriptor);
