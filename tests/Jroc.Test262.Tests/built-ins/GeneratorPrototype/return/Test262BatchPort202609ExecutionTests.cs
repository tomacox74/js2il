...
{
    public Test262BatchPort202609ExecutionTests() : base("built_ins.GeneratorPrototype.return") { }

    [Fact(DisplayName = "from-state-completed")]
    public Task from_state_completed()
        => ExecutionTestFromFile("from-state-completed");

    [Fact(DisplayName = "from-state-suspended-start")]
    public Task from_state_suspended_start()
        => ExecutionTestFromFile("from-state-suspended-start");
    ...
    [Fact(DisplayName = "try-finally-set-property-within-try")]
    public Task try_finally_set_property_within_try()
        => ExecutionTestFromFile("try-finally-set-property-within-try");
