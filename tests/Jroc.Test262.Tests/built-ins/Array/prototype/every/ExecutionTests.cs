using Jroc.Test262.Tests.built_ins;



namespace Jroc.Test262.Tests.built_ins.Array.prototype.every;



public partial class ExecutionTests : InMemoryExecutionTestsBase

{

    public ExecutionTests() : base("built_ins.Array.prototype.every") { }



    [Fact(DisplayName = "15.4.4.16-0-1")]

    public Task _15_4_4_16_0_1()

        => ExecutionTestFromFile("15.4.4.16-0-1");



    [Fact(DisplayName = "15.4.4.16-1-1")]

    public Task _15_4_4_16_1_1()

        => ExecutionTestFromFile("15.4.4.16-1-1");



    [Fact(DisplayName = "15.4.4.16-1-10")]

    public Task _15_4_4_16_1_10()

        => ExecutionTestFromFile("15.4.4.16-1-10");



    [Fact(DisplayName = "15.4.4.16-1-11")]
    public Task _15_4_4_16_1_11()

        => ExecutionTestFromFile("15.4.4.16-1-11");



    [Fact(DisplayName = "15.4.4.16-1-12")]

    public Task _15_4_4_16_1_12()

        => ExecutionTestFromFile("15.4.4.16-1-12");



    [Fact(DisplayName = "15.4.4.16-1-13")]

    public Task _15_4_4_16_1_13()

        => ExecutionTestFromFile("15.4.4.16-1-13");



    [Fact(DisplayName = "15.4.4.16-1-14")]

    public Task _15_4_4_16_1_14()

        => ExecutionTestFromFile("15.4.4.16-1-14");



    [Fact(DisplayName = "15.4.4.16-1-15")]

    public Task _15_4_4_16_1_15()

        => ExecutionTestFromFile("15.4.4.16-1-15");



    [Fact(DisplayName = "15.4.4.16-1-2")]

    public Task _15_4_4_16_1_2()

        => ExecutionTestFromFile("15.4.4.16-1-2");



    [Fact(DisplayName = "15.4.4.16-1-3")]

    public Task _15_4_4_16_1_3()

        => ExecutionTestFromFile("15.4.4.16-1-3");



    [Fact(DisplayName = "15.4.4.16-1-4")]

    public Task _15_4_4_16_1_4()

        => ExecutionTestFromFile("15.4.4.16-1-4");



    [Fact(DisplayName = "15.4.4.16-1-5")]

    public Task _15_4_4_16_1_5()

        => ExecutionTestFromFile("15.4.4.16-1-5");



    [Fact(DisplayName = "15.4.4.16-1-6")]

    public Task _15_4_4_16_1_6()

        => ExecutionTestFromFile("15.4.4.16-1-6");



    [Fact(DisplayName = "15.4.4.16-1-7")]

    public Task _15_4_4_16_1_7()

        => ExecutionTestFromFile("15.4.4.16-1-7");



    [Fact(DisplayName = "15.4.4.16-1-8")]

    public Task _15_4_4_16_1_8()

        => ExecutionTestFromFile("15.4.4.16-1-8");



    [Fact(DisplayName = "15.4.4.16-1-9")]

    public Task _15_4_4_16_1_9()

        => ExecutionTestFromFile("15.4.4.16-1-9");

    [Fact(DisplayName = "15.4.4.16-2-1")]
    public Task _15_4_4_16_2_1()
        => ExecutionTestFromFile("15.4.4.16-2-1");

    [Fact(DisplayName = "15.4.4.16-2-10")]
    public Task _15_4_4_16_2_10()
        => ExecutionTestFromFile("15.4.4.16-2-10");

    [Fact(DisplayName = "15.4.4.16-2-11")]
    public Task _15_4_4_16_2_11()
        => ExecutionTestFromFile("15.4.4.16-2-11");

    [Fact(DisplayName = "15.4.4.16-2-13")]
    public Task _15_4_4_16_2_13()
        => ExecutionTestFromFile("15.4.4.16-2-13");

    [Fact(DisplayName = "15.4.4.16-3-22")]
    public Task _15_4_4_16_3_22()
        => ExecutionTestFromFile("15.4.4.16-3-22");

}
