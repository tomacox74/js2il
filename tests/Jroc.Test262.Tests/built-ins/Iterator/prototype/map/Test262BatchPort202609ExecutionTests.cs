...
{
    public Test262BatchPort202609ExecutionTests() : base("built_ins.Iterator.prototype.map") { }

    [Fact(DisplayName = "argument-effect-order")]
    public Task argument_effect_order()
        => ExecutionTestFromFile("argument-effect-order");

    [Fact(DisplayName = "argument-validation-failure-closes-underlying")]
    public Task argument_validation_failure_closes_underlying()
        => ExecutionTestFromFile("argument-validation-failure-closes-underlying");

    [Fact(DisplayName = "exhaustion-does-not-call-return")]
    public Task exhaustion_does_not_call_return()
        => ExecutionTestFromFile("exhaustion-does-not-call-return");
    ...
    public Task get_return_method_throws()
        => ExecutionTestFromFile("get-return-method-throws");

    [Fact(DisplayName = "iterator-return-method-throws")]
    public Task iterator_return_method_throws()
        => ExecutionTestFromFile("iterator-return-method-throws");

    [Fact(DisplayName = "mapper-throws-then-closing-iterator-also-throws")]
    public Task mapper_throws_then_closing_iterator_also_throws()
        => ExecutionTestFromFile("mapper-throws-then-closing-iterator-also-throws");

    [Fact(DisplayName = "mapper-throws")]
    public Task mapper_throws()
        => ExecutionTestFromFile("mapper-throws");
    ...
    public Task return_is_forwarded_to_underlying_iterator()
        => ExecutionTestFromFile("return-is-forwarded-to-underlying-iterator");

    [Fact(DisplayName = "return-is-not-forwarded-after-exhaustion")]
    public Task return_is_not_forwarded_after_exhaustion()
        => ExecutionTestFromFile("return-is-not-forwarded-after-exhaustion");

    [Fact(DisplayName = "this-non-callable-next")]
    public Task this_non_callable_next()
        => ExecutionTestFromFile("this-non-callable-next");
    ...
    public Task throws_typeerror_when_generator_is_running()
        => ExecutionTestFromFile("throws-typeerror-when-generator-is-running");
