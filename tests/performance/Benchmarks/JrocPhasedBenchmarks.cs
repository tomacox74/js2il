...
public class JrocPhasedBenchmarks
{
    ...
    public void Setup()
    {
        ...
        foreach (var kvp in _scripts)
        {
            ...
            try
            {
                ...
                {
                    _jrocCompileFailures[scenarioKey] = "jroc compilation failed for this scenario";
                    continue;
                }
                ...
                {
                    _jrocCompileFailures[scenarioKey] = $"compiled assembly not found: {dllPath}";
                    continue;
                }
                ...
                var loadContext = new BenchmarkModuleLoadContext(
                    ...
                    fullDllPath,
                    $"jroc-bench-{scriptName}-{Guid.NewGuid():N}");
            ...
            {
                _jrocCompileFailures[scenarioKey] = $"setup failed: {ex.Message}";
            }
        }
    }
    ...
    [Benchmark(Description = "jroc compile")]
    public void Jroc_Compile()
    {
        ...
        var tempOutputDir = Path.Combine(Path.GetTempPath(), $"jroc-compile-{Guid.NewGuid()}");
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException($"jroc compile benchmark failed for scenario '{resolvedScriptName}'.");
            }
        }
        finally
        {
    ...
    [Benchmark(Description = "jroc execute (pre-compiled)")]
    public void Jroc_ExecuteOnly()
    {
        if (_jrocCompileFailures.TryGetValue(ScriptName, out var failure))
        {
            throw new InvalidOperationException(
                $"jroc phased setup failed for scenario '{ResolveScriptName(ScriptName)}': {failure}");
        }
