...
public class IRPipelineAuditTests : IDisposable
{
    ...
    {
        _output = output;
        _outputPath = Path.Combine(Path.GetTempPath(), "Jroc.Tests", "IRPipelineAudit");
    ...
    public void FullSuiteIRPipelineMetrics()
    {
        ...
        foreach (var resourceName in resourceNames)
        {
            try
            {
                ...
                var resourceWithoutPrefix = resourceName.StartsWith("Jroc.Tests.", StringComparison.Ordinal)
                    ? resourceName.Substring("Jroc.Tests.".Length)
                    : resourceName;
        ...
        if (failures.Count > 0 && failures.Count <= 10)
        {
            ...
            {
                var shortName = resource.Replace("Jroc.Tests.", "");
    ...
    private void CompileJavaScript(string js, string testName)
    {
        ...
        mockFileSystem.AddFile(testFilePath, "\"use strict\";\n\n" + js);
