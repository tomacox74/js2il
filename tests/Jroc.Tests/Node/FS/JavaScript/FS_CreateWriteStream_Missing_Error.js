...
const missingDir = path.join(os.tmpdir(), `jroc-fs-write-stream-missing-${uniqueSuffix}`, 'nested');
const file = path.join(missingDir, 'file.txt');
fs.rmSync(path.join(os.tmpdir(), `jroc-fs-write-stream-missing-${uniqueSuffix}`), { force: true, recursive: true });
