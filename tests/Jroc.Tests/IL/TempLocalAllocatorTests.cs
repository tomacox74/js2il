...
{
    [Fact]
    public void Allocate_IdentitySchedule_MatchesLegacyAllocation()
    {
        var body = CreateSequentialCallBody();
        var legacy = TempLocalAllocator.Allocate(body);
        var schedule = LIRStackScheduler.Identity(body);
        var plan = CreatePlan(body, schedule);

        var scheduled = TempLocalAllocator.Allocate(body, plan, schedule);

        Assert.Equal(legacy.TempToSlot, scheduled.TempToSlot);
        Assert.Equal(legacy.SlotStorages, scheduled.SlotStorages);
    }
