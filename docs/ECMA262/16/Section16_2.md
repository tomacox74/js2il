...
> Last generated (UTC): 2026-08-13T14:32:07Z

JROC supports a practical subset of module syntax by lowering top-level `import` / `export` declarations to the existing CommonJS runtime model and aligning literal dynamic `import()` with the same pragmatic namespace/default interop surface. The compiler now also builds explicit module-record metadata, performs separate link/evaluation planning passes, resolves re-export graphs with cycle/ambiguity checks before IL generation, and lowers top-level `await` in compiled module bodies through the async state-machine pipeline. Namespace objects, re-exports, common cyclic/live-binding cases, and root-module top-level await work via pragmatic runtime interop, but host loading hooks, full async module dependency-graph evaluation semantics, and import attributes remain unsupported.
...
|---|---|---|---|---|
| Full module record/linking/evaluation model | Supported with Limitations | [`Import_LiveBindings_Cycle.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_LiveBindings_Cycle.js)<br>[`Import_ExportStarFrom.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_ExportStarFrom.js)<br>[`Import_ExportStarFromMultiHop.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_ExportStarFromMultiHop.js) |  | JROC now materializes explicit module-record metadata, performs a separate graph-based linking pass, and plans evaluation order/components before IL generation. This covers module-request records, graph-driven `GetExportedNames` / `ResolveExport`, common cyclic graphs, and link-time ambiguity/missing-export diagnostics for the supported static ESM subset. The runtime still executes through the CommonJS-oriented host model, so host loading hooks, true `ParseModule`, and full async module dependency-graph algorithms remain unsupported. |

### 16.2.1.5 ([tc39.es](https://tc39.es/ecma262/#sec-abstract-module-records))

| Feature name | Status | Test scripts | test262 evidence | Notes |
|---|---|---|---|---|
| Top-level await in module bodies | Supported with Limitations | [`Async_TopLevelAwait_ResolvedPromise.js`](../../../tests/Jroc.Tests/Async/JavaScript/Async_TopLevelAwait_ResolvedPromise.js)<br>[`Async_TopLevelAwait_PendingPromise.js`](../../../tests/Jroc.Tests/Async/JavaScript/Async_TopLevelAwait_PendingPromise.js)<br>[`Async_TopLevelAwait_RejectionCaught.js`](../../../tests/Jroc.Tests/Async/JavaScript/Async_TopLevelAwait_RejectionCaught.js) |  | Top-level `await` parses successfully and lowers through an async module-body state machine. The generated CommonJS module wrapper waits for the returned Promise before marking the entry module complete, including pending Promise resumption and caught rejection paths. This is not a full SourceTextModuleRecord async-evaluation implementation yet, so cross-module async dependency coordination remains limited by the existing CommonJS-oriented module runtime. |
...
| Feature name | Status | Test scripts | test262 evidence | Notes |
|---|---|---|---|---|
| Module namespace objects for static imports | Supported with Limitations | [`Import_Namespace_Esm_Basic.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_Namespace_Esm_Basic.js)<br>[`Import_Namespace_FromCjs_Stable.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_Namespace_FromCjs_Stable.js) |  | Namespace objects are produced through the `__jroc_esm_namespace` helper. ESM namespace imports expose the export object directly, while CommonJS namespace imports use a cached synthetic namespace object with getter-based forwarding. |

### 16.2.1.14 ([tc39.es](https://tc39.es/ecma262/#sec-module-semantics-runtime-semantics-evaluation))

| Feature name | Status | Test scripts | test262 evidence | Notes |
|---|---|---|---|---|
| Module body evaluation via CommonJS require and export getters | Supported with Limitations | [`Import_RequireEsmModule.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_RequireEsmModule.js)<br>[`Import_LiveBindings_Cycle.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_LiveBindings_Cycle.js)<br>[`Import_ExportStarFrom.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_ExportStarFrom.js) |  | Module bodies still evaluate through generated CommonJS `require(...)` calls and getter-based export wiring, but the compiler now plans evaluation order/components from the linked module graph before code generation. Common cyclic/live-binding cases, multi-hop re-export chains, and root-module top-level `await` are covered for the supported static ESM subset. Full async module dependency-graph execution remains limited by the CommonJS-oriented runtime. |

### 16.2.2 ([tc39.es](https://tc39.es/ecma262/#sec-imports))

| Feature name | Status | Test scripts | test262 evidence | Notes |
|---|---|---|---|---|
| Dynamic import() namespace/default interop | Supported with Limitations | [`Import_DynamicImport_Esm_Namespace.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_DynamicImport_Esm_Namespace.js)<br>[`Import_DynamicImport_Cjs_Namespace.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_DynamicImport_Cjs_Namespace.js) |  | Literal dynamic `import()` requests resolve through the existing CommonJS loader but now return the same pragmatic namespace/default projection used by the static ESM lowering. ESM-lowered modules resolve to their export object directly; CommonJS-style modules resolve to a cached synthetic namespace object that exposes `default`, `module.exports`, and getter-backed enumerable export keys. Full ECMA-262 module-record linking/evaluation semantics remain out of scope and are tracked separately. |
| Top-level static import declarations | Supported with Limitations | [`Import_StaticImport_FromCjs.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_StaticImport_FromCjs.js)<br>[`Import_LiveBindings_Named.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_LiveBindings_Named.js)<br>[`Import_LiveBindings_Cycle.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_LiveBindings_Cycle.js)<br>[`Import_Namespace_Esm_Basic.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_Namespace_Esm_Basic.js)<br>[`Import_Namespace_FromCjs_Stable.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_Namespace_FromCjs_Stable.js) |  | Supported forms include side-effect imports, default imports, named imports, and namespace imports. Static `import`/`export` modules are lowered natively: no implementation-generated JavaScript is injected before compilation. Source module dependencies resolve through the CommonJS loader (`require(...)`), while imported identifier reads compile to live reads directly against the resolved source module. Namespace imports from ESM return the module export object; namespace imports from CommonJS return a cached synthetic namespace object whose properties forward through to the underlying exports (live values for keys present at namespace creation time). Imported bindings are treated as immutable; assignment/update/destructuring writes and `delete` of an imported binding are rejected at compile time. The compiler validates ESM named/default imports against the linked module graph and reports missing or ambiguous re-export chains during linking. Static import/export declarations must still appear at top level and before non-directive top-level statements, and `with` clauses / import attributes are rejected. |

### 16.2.3 ([tc39.es](https://tc39.es/ecma262/#sec-exports))

| Feature name | Status | Test scripts | test262 evidence | Notes |
|---|---|---|---|---|
| Top-level export declarations | Supported with Limitations | `tests/Jroc.Tests/Import/JavaScript/Import_RequireEsmModule_Lib.mjs`<br>`tests/Jroc.Tests/Import/JavaScript/Import_LiveBindings_Named_Lib.mjs`<br>`tests/Jroc.Tests/Import/JavaScript/Import_LiveBindings_Cycle_A.mjs`<br>`tests/Jroc.Tests/Import/JavaScript/Import_LiveBindings_Cycle_B.mjs`<br>[`Import_ExportNamedFrom.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_ExportNamedFrom.js)<br>[`Import_ExportRenamedFrom.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_ExportRenamedFrom.js)<br>[`Import_ExportStarFrom.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_ExportStarFrom.js)<br>[`Import_ExportStarFromMultiHop.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_ExportStarFromMultiHop.js)<br>[`Import_ExportStarAsNamespace.js`](../../../tests/Jroc.Tests/Import/JavaScript/Import_ExportStarAsNamespace.js)<br>`tests/Jroc.Tests/Import/JavaScript/Import_DefaultAnonFunction_Lib.mjs`<br>`tests/Jroc.Tests/Import/JavaScript/Import_DefaultAnonClass_Lib.mjs`<br>`tests/Jroc.Tests/Import/JavaScript/Import_DefaultNamedClass_Lib.mjs`<br>`tests/Jroc.Tests/Import/JavaScript/Import_Namespace_Esm_Basic_Lib.mjs`<br>`tests/Jroc.Tests/Import/JavaScript/Import_Namespace_FromCjs_Stable_Lib.cjs` |  | Supported forms include named/default exports (named, anonymous, and expression `export default`), `export { x as y } from`, `export *`, and `export * as ns from` re-exports. Static `import`/`export` modules are lowered natively with no injected JavaScript helper source: each local export is backed by a runtime binding cell exposed as a live, enumerable accessor on the module's `exports` object, exported writes mirror into that cell, indirect/star/namespace re-exports install live forwarding accessors, and anonymous default function/class declarations synthesize their default binding in compiler metadata. Live values, temporal-dead-zone `ReferenceError`s, and CommonJS interop are preserved. The runtime still executes through the CommonJS-oriented host model, so host hooks, async module execution, and full SourceTextModuleRecord environment semantics remain out of scope. |
