...
| 27.1.3.2.1 | Iterator.concat ( ... items ) | Supported with Limitations | [tc39.es](https://tc39.es/ecma262/#sec-iterator.concat) |
...
| Feature name | Status | Test scripts | test262 evidence | Notes |
|---|---|---|---|---|
| Iterator helper objects support lazy map/filter/drop/take/flatMap pipelines plus helper next/return behavior | Supported with Limitations | [`Iterator_From_HelperChain.js`](../../../tests/Jroc.Tests/Iterator/JavaScript/Iterator_From_HelperChain.js)<br>[`Iterator_Helper_Next_Return.js`](../../../tests/Jroc.Tests/Iterator/JavaScript/Iterator_Helper_Next_Return.js) |  | Helper objects are lazy, iterable, and expose `next()` / `return()` through the public iterator-helper surface. Iterator records acquire `next` once, validate iterator-result objects, preserve `done`/`value` access order, forward `return` lazily, apply IteratorClose completion precedence, and reject reentrant execution. Iterator, @@iterator, Symbol.dispose, drop, every, and the remaining helper functions expose their covered standard metadata; only the synchronous helper family is implemented here. |

### 27.1.3 ([tc39.es](https://tc39.es/ecma262/#sec-iterator-objects))

| Feature name | Status | Test scripts | test262 evidence | Notes |
|---|---|---|---|---|
| Public Iterator surface exposes Iterator.from and the helper methods for ordinary synchronous iterables | Supported with Limitations | [`Iterator_From_HelperChain.js`](../../../tests/Jroc.Tests/Iterator/JavaScript/Iterator_From_HelperChain.js)<br>[`Iterator_Helper_Next_Return.js`](../../../tests/Jroc.Tests/Iterator/JavaScript/Iterator_Helper_Next_Return.js) |  | JROC exposes global `Iterator` and `Iterator.prototype` surfaces, including `Iterator.from(...)`, `Iterator.concat(...)`, `Iterator.zip(...)`, `Iterator.zipKeyed(...)`, helper chaining, terminal helpers, and iterable array-iterator objects. Full abstract-constructor and cross-realm semantics are still limited. |

### 27.1.3.2 ([tc39.es](https://tc39.es/ecma262/#sec-properties-of-the-iterator-constructor))

| Feature name | Status | Test scripts | test262 evidence | Notes |
|---|---|---|---|---|
| Iterator sequencing and joint iteration | Supported with Limitations | `tests/Jroc.Test262.Tests/built-ins/Iterator/concat/ExecutionTests.cs`<br>`tests/Jroc.Test262.Tests/built-ins/Iterator/zip/ExecutionTests.cs`<br>`tests/Jroc.Test262.Tests/built-ins/Iterator/zipKeyed/ExecutionTests.cs` | `test/built-ins/Iterator/concat/return-is-forwarded.js`<br>`test/built-ins/Iterator/zip/basic-longest.js`<br>`test/built-ins/Iterator/zipKeyed/basic-shortest.js` | 100 pinned Test262 cases that failed before implementation now verify Iterator.concat and the joint-iteration Iterator.zip and Iterator.zipKeyed methods, including options, padding, iterator acquisition, abrupt completion, metadata, and iterator closing. Cross-realm and a few remaining iterator edge cases are not covered. |
