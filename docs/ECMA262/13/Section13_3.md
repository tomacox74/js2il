...
| NewExpression for user-defined classes (new Ctor(...)) | Supported | [`Classes_DeclareEmptyClass.js`](../../../tests/Jroc.Tests/Classes/JavaScript/Classes_DeclareEmptyClass.js)<br>[`Classes_ClassConstructor_WithMultipleParameters.js`](../../../tests/Jroc.Tests/Classes/JavaScript/Classes_ClassConstructor_WithMultipleParameters.js) | User-defined classes are lowered to direct CLR construction of the generated type, including scope-array wiring for closures in constructors/methods. Spread arguments in `new` are not supported. |
| NewExpression where the constructor is a runtime value (new valueCtor(...)) | Supported |  | When the constructor is not statically known, JROC lowers to a runtime dispatch (`JavaScriptRuntime.Object.ConstructValue`) by packing arguments into an object[]. Spread arguments in `new` are not supported. |
