...
> Last generated (UTC): 2026-09-25T10:22:45Z
...
| Feature name | Status | Test scripts | test262 evidence | Notes |
|---|---|---|---|---|
| Assignment-target early-error conformance expansion | Supported with Limitations | `tests/Jroc.Test262.Tests/language/expressions/ExpressionSyntaxConformance7BatchParseTests.cs` |  | The native Test262 harness verifies 96 additional pinned parse-negative fixtures: five invalid assignment targets and 91 invalid parenthesized or expression-form assignment targets. They are preserved upstream fixtures and require no compiler, runtime, or harness change. |
| Computed property/index assignment (obj[index] = value; arr[i] = value) | Supported | [`Variable_AssignmentTargets_MemberAndIndex.js`](../../../tests/Jroc.Tests/Variable/JavaScript/Variable_AssignmentTargets_MemberAndIndex.js) | `test/language/expressions/assignment/target-member-computed-reference.js` | Supports computed MemberExpression assignment via JavaScriptRuntime.Object.SetItem(obj, index, value). Works for object literals (ExpandoObject), JavaScriptRuntime.Array index writes (including extending length), and Int32Array index writes. Simple assignments now preserve the spec-required evaluation order by resolving the base reference and computed key before evaluating the right-hand side. |
...
| Property assignment on objects (obj.prop = value) | Supported | [`ObjectLiteral_PropertyAssign.js`](../../../tests/Jroc.Tests/Literals/JavaScript/ObjectLiteral_PropertyAssign.js)<br>[`Variable_AssignmentTargets_MemberAndIndex.js`](../../../tests/Jroc.Tests/Variable/JavaScript/Variable_AssignmentTargets_MemberAndIndex.js)<br>[`CommonJS_Module_Exports_ChainedAssignment.js`](../../../tests/Jroc.Tests/CommonJS/JavaScript/CommonJS_Module_Exports_ChainedAssignment.js)<br>[`CommonJS_Module_Exports_ChainedAssignment_Lib.js`](../../../tests/Jroc.Tests/CommonJS/JavaScript/CommonJS_Module_Exports_ChainedAssignment_Lib.js) | `test/language/expressions/assignment/11.13.1-4-28gs.js`<br>`test/language/expressions/assignment/11.13.1-4-29gs.js`<br>`test/language/expressions/assignment/11.13.1-4-6-s.js`<br>`test/language/expressions/assignment/8.14.4-8-b_2.js` | Supports assignment to non-computed member targets and returns the assigned value (JavaScript assignment-expression semantics), including chained assignment patterns where the result is consumed (e.g., exports = module.exports = {...}; fixes #558). Emitted via JavaScriptRuntime.Object.SetItem(obj, key, value) with a string key. Supports ExpandoObject (object literals) and reflection-backed host objects; strings are treated as immutable and ignore writes. Strict assignment to read-only intrinsic, built-in, and inherited data properties such as Math.PI, Function.length, and non-writable prototype properties throws TypeError. |
| Test262 assignment-target early-error conformance batch | Supported with Limitations | `tests/Jroc.Test262.Tests/language/expressions/assignmenttargettype/ExpressionSyntaxConformanceBatchParseTests.cs` |  | 148 pinned Test262 assignment-target fixtures are natively verified as parse-negative cases, covering invalid direct and parenthesized assignment targets across the expression grammar. |
...
| Compound assignments to private fields and accessors | Supported with Limitations | `tests/Jroc.Test262.Tests/language/expressions/compound-assignment/ExecutionTests.cs` | `test/language/expressions/compound-assignment/left-hand-side-private-reference-data-property-add.js`<br>`test/language/expressions/compound-assignment/left-hand-side-private-reference-accessor-property-div.js` | Private compound assignments read the initial private value, evaluate the right-hand side once, apply the operator, and write the result through the private field or setter. Object environment records created by with statements still have separate reference-preservation limitations. |
| Logical assignment reference preservation and NamedEvaluation | Supported | `tests/Jroc.Test262.Tests/language/expressions/logical-assignment/ExecutionTests.cs`<br>`tests/Jroc.Test262.Tests/language/expressions/assignment/ExecutionTests.cs`<br>`tests/Jroc.Test262.Tests/language/expressions/logical-assignment/ExpressionSyntaxConformanceBatchExecutionTests.cs`<br>`tests/Jroc.Test262.Tests/language/expressions/logical-assignment/ExpressionSyntaxConformanceBatchParseTests.cs` |  | &&=, \|\|=, and ??= short-circuit without invoking private setters or writing private fields when the existing value determines the result. The left-hand reference and receiver are evaluated once, anonymous function/class RHS values receive inferred names, and assignment through with-environment references updates the resolved binding. The native Test262 harness additionally verifies 34 logical-assignment fixtures. |
| Test262 compound-assignment conformance batch | Supported with Limitations | `tests/Jroc.Test262.Tests/language/expressions/compound-assignment/CompoundAssignmentConformanceBatchExecutionTests.cs`<br>`tests/Jroc.Test262.Tests/language/expressions/compound-assignment/CompoundAssignmentConformanceBatchParseTests.cs`<br>`tests/Jroc.Test262.Tests/language/expressions/compound-assignment/ExpressionSyntaxConformanceBatchExecutionTests.cs`<br>`tests/Jroc.Test262.Tests/language/expressions/compound-assignment/ExpressionSyntaxConformanceBatchParseTests.cs` |  | 355 pinned Test262 compound-assignment fixtures are natively verified: 321 positive execution cases and 34 parse-negative cases. The batch covers arithmetic, bitwise, and shift compound assignments, primitive coercion, strict-mode early errors, and private-field targets. |
