...
- benchmarks/phased: temporarily exclude `evaluation`, `evaluation-modern`, and `linq-js` from `JrocPhasedBenchmarks` scenario discovery while those scenarios are investigated for fix-or-remove follow-up.
...
- benchmarks/perf: fix `JrocPhasedBenchmarks` scenario keying to use plain script names (matching BenchmarkDotNet `ScriptName`) so phased runs no longer hit dictionary `KeyNotFoundException` and produce `NA`-only results.
